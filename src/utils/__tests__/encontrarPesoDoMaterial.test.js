...
describe('encontrarPesoDoMaterial', () => {
  ...
  it('uses tabela aço when material normalizes to aço', () => {
    ...
    expect(encontrarPesoDoMaterial(16.17, 'Aço')).toBe(2.24)
    expect(encontrarPesoDoMaterial(16.17, 'aço')).toBe(2.24)
  })

  it('uses tabela aluminio for non-aço materials', () => {
    expect(encontrarPesoDoMaterial(10, 'Aluminio')).toBe(13.42)
    expect(encontrarPesoDoMaterial(10, 'aluminio')).toBe(13.42)
  })

  it('returns first matching valor strictly greater than diameter', () => {
    expect(encontrarPesoDoMaterial(6.35, 'Aço')).toBe(0.99)
    expect(encontrarPesoDoMaterial(6.34, 'Aço')).toBe(0.25)
  })

  it('returns undefined when no table row has diametro > recebido', () => {
    expect(encontrarPesoDoMaterial(1e9, 'Aço')).toBeUndefined()
    expect(encontrarPesoDoMaterial(1e9, 'Aluminio')).toBeUndefined()
  })
})
